...
def get_visit_pts(mimic4_path:str, group_col:str, visit_col:str, admit_col:str, disch_col:str, adm_visit_col:str, use_mort:bool, disease_label:str,use_ICU:bool):
...
def extract_data(use_ICU:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None):
    ...
    if not cohort_output:
        cohort_output="cohort_" + use_ICU.lower() + "_mortality_" + str(time) + "_" + disease_label
    if not summary_output:
        summary_output="summary_" + use_ICU.lower() + "_mortality_" + str(time) + "_" + disease_label
    
    if icd_code=="No Disease Filter":
        if len(disease_label):
            print(f"EXTRACTING FOR: | {use_ICU.upper()} | MORTALITY DUE TO {str(disease_label).upper()} | {str(time)} | ")
        else:
            print(f"EXTRACTING FOR: | {use_ICU.upper()} | MORTALITY | {str(time)} |")
    else:
        if len(disease_label):
            print(f"EXTRACTING FOR: | {use_ICU.upper()} | MORTALITY DUE TO {str(disease_label).upper()} | ADMITTED DUE TO {icd_code.upper()} | {str(time)} |")
        else:
            print(f"EXTRACTING FOR: | {use_ICU.upper()} | MORTALITY | ADMITTED DUE TO {icd_code.upper()} | {str(time)} |")
    #print(label)
    ...
    use_mort = True # Focus only on mortality
